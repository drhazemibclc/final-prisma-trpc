...
export const ViewAppointment = async ({ id }: ViewAppointmentProps) => {
    ...
    const caller = await createCaller({
        req: new Request("http://localhost"), // dummy request (safe fallback)
        headers: await headers(),
        cookies: await cookies(),
        session,
    });

    const data = await caller.appointment.getAppointmentById({ id });
    if (!data) return null;
        ...
        <Dialog>
            ...
            <DialogContent className="max-h-[95%] max-w-[425px] overflow-y-auto p-8 md:max-w-2xl 2xl:max-w-3xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        This appointment was booked on the {formatDateTime(data?.data?.createdAt?.toString() ?? "")}
                    </DialogDescription>
                </DialogHeader>
