...
function SidebarProvider({
    ...
    // This effect runs once on mount to read the initial cookie state
    React.useEffect(() => {
        const readCookie = async () => {
            if (typeof window !== 'undefined' && 'cookieStore' in navigator) {
                ...
                // Fallback for browsers without Cookie Store API support (or SSR)
                const name = `${SIDEBAR_COOKIE_NAME}=`;
                const decodedCookie = decodeURIComponent(document.cookie);
                const ca = decodedCookie.split(';');

                // The old for loop was replaced with this for...of loop.
                for (const c of ca) {
    ...
    const setOpen = React.useCallback(
        async (value: boolean | ((value: boolean) => boolean)) => {
            ...
            // Set the cookie using the Cookie Store API
            if (typeof window !== 'undefined' && 'cookieStore' in navigator) {
                try {
                    await navigator.cookieStore.set({
                        name: SIDEBAR_COOKIE_NAME,
                        value: String(openState), // Store boolean as string "true" or "false"
                        expires: Date.now() + SIDEBAR_COOKIE_MAX_AGE * 1000, // max-age in seconds, expires in milliseconds
                        path: '/'
                        // Consider 'secure: true' if your site is HTTPS
                        // Consider 'sameSite: "Lax"' for default security
                    });
                } catch (error) {
