...
import { createTRPCRouter, protectedProcedure } from "@/server/api/trpc"; // or your tRPC init path
...
export const appointmentRouter = createTRPCRouter({
    getAppointmentById: protectedProcedure.input(z.object({ id: z.number() })).query(async ({ input }) => {
    ...
    createNewAppointment: protectedProcedure.input(AppointmentInputSchema).mutation(async ({ input }) => {
        const result = await createNewAppointment(input);
        if (!result.success) {
            throw new Error(result.msg || "Failed to create appointment");
        }
        return result;
    }),

    appointmentAction: protectedProcedure.input(AppointmentActionInputSchema).mutation(async ({ input }) => {
        const { id, status, reason = "" } = input;
        const result = await appointmentAction(id, status as AppointmentStatus, reason);
        if (!result.success) {
            throw new Error(result.msg || "Failed to update appointment status");
        }
        return result;
    }),
    ...
    addVitalSigns: protectedProcedure.input(AddVitalSignsInputSchema).mutation(async ({ input }) => {
        ...
        if (!result.success) {
            throw new Error(result.msg || "Failed to add vital signs");
        }
        return result;
    }),
});
