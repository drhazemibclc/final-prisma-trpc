...
export async function getDoctors() {
    try {
        ...
        return { success: true, data, status: MAGIC_NUMBER_1 };
    } catch (error) {
        console.log(error);
        return {
            success: false,
...
export async function getDoctorDashboardStats() {
    try {
        ...
        const last5Records = appointments.slice(0, MAGIC_NUMBER_3);
        // const availableDoctors = doctors.slice(0, MAGIC_NUMBER_3);
...
export async function getRatingById(id: string) {
    try {
        ...
        const formattedRatings = (Math.round(averageRating * MAGIC_NUMBER_4) / MAGIC_NUMBER_4).toFixed(1);
...
export async function getAllDoctors({
    ...
    try {
        ...
        const LIMIT = Number(limit) || MAGIC_NUMBER_4;
